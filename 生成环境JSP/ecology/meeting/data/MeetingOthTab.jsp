...
<%
	...
	if("TopicDate".equals(toflag)){
	...
	} else if("TopicAttach".equals(toflag)){
		url = "/meeting/data/MeetingTopicAttach.jsp";
		navName = SystemEnv.getHtmlLabelName(2210, user.getLanguage())+(user.getLanguage()==8?" ":"")+SystemEnv.getHtmlLabelNames("2169,156", user.getLanguage());
		...
		navName = SystemEnv.getHtmlLabelName(2103, user.getLanguage())+(user.getLanguage()==8?" ":"")+SystemEnv.getHtmlLabelName(430, user.getLanguage())+(user.getLanguage()==8?" ":"")+SystemEnv.getHtmlLabelName(2108, user.getLanguage());
	}else if("newReHrm".equals(toflag)){
		url = "/weavernorth/meeting/MeetingReHrm.jsp";
		navName = SystemEnv.getHtmlLabelName(2103, user.getLanguage())+(user.getLanguage()==8?" ":"")+SystemEnv.getHtmlLabelName(430, user.getLanguage())+(user.getLanguage()==8?" ":"")+SystemEnv.getHtmlLabelName(2108, user.getLanguage());
	...
	}else if("Inform".equals(toflag)){ //通知阅读情况
		url = "/meeting/data/MeetingInform.jsp";
		navName = "通知阅读情况";
	}else if("SignIn".equals(toflag)){ //会议签到情况
		url = "/meeting/data/MeetingSignIn.jsp";
		navName = "会议签到情况";
	...
	}else if("MeetingSign".equals(toflag)){
		url = "/meeting/data/MeetingParticipants.jsp";
		navName = "会议签到情况";
	}else {
		out.println("<script>wfforward(\"/notice/noright.jsp\");</script>");
		return;
