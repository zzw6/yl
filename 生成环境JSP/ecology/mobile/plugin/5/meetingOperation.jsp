...
<%
	...
	try{
		...
		if("getMeetingNum".equals(operation)){
			...
			sb.append(" and m.address = a.id and m.repeatType = 0");
			...
			if(!"".equals(address)&&!"".equals(a)&&!"".equals(b)&&!"".equals(c)&&!"".equals(d)){
				...
				sb.append(" and m.address = "+address+" and m.repeatType = 0");
				sb.append(" and (("+bds+" <='"+bdstr+"' and "+eds+" >='"+bdstr+"')");
				sb.append(" or ("+bds+">='"+bdstr+"' and "+bds+" <='"+edstr+"'))");
				if(!"".equals(meetingid)){
			...
			rs.executeSql("select m.*,r.name as roomname from meeting m left join MeetingRoom r on m.address = r.id"
					+" where m.creater = "+userid
			...
			if(!"".equals(addressid)&&currentDay!=0){
				...
				rs.executeSql("select t1.*,t3.name as roomname from Meeting t1 left join MeetingRoom t3 on t1.address = t3.id "+
					"where t1.address = "+addressid+" and t1.beginDate <= '"+
...
<%!
	private String getSql(String subcompanyid,User user,String beginDate,String endDate,int type){
		...
		sb.append(" from Meeting t1 left join MeetingRoom t3 on t1.address = t3.id,Meeting_ShareDetail t2");
		sb.append(" where t1.id = t2.meetingId and t1.repeatType = 0 and");
		// 判断是否添加议事规则查询条件
