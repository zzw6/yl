...
{	
	...
	if(topicAttachrows != mtaCount || topicrows != mtcount){
		...
		String xinzengDoc = TimeUtils.replaceRepStr(docall.toString());
		
		String meetingName = "";
		String t_recorder = "";	//记录人
		String t_ccmeetingnotice = "";	//会议纪要抄送人
		String t_caller = "";	//主持人
		String t_hrmmembers = "";	//参会人员
		RecordSet.executeSql("select name,recorder,ccmeetingnotice,caller,hrmmembers from meeting where id = " + meetingid);
		if(RecordSet.next()){
			meetingName = RecordSet.getString("name");
			t_recorder = RecordSet.getString("recorder");
			t_ccmeetingnotice = RecordSet.getString("ccmeetingnotice");
			t_caller = RecordSet.getString("caller");
			t_hrmmembers = RecordSet.getString("hrmmembers");
		}
		
		String tempUsers = t_recorder + "," + t_ccmeetingnotice + "," + t_caller + "," + t_hrmmembers;
		
		String t_ccMeetingNotice = TimeUtils.replaceRepStr(tempUsers);
		
		String description11 = "邮件通知：XXX会议 议程资料变更通知";
		String mailTitle = description11.replaceAll("XXX",meetingName);
		String mailContent = "以下是提醒内容，请点击查看详情：<br>";
		mailContent += "<a style=\"color:red\" target=\"_blank\" href=\"/weavernorth/meeting/MeetingInfo.jsp?fujian="+xinzengDoc+"&id="+meetingid+"\">"+mailTitle+"</a><br>";
		
		MailSend localMailSend = new MailSend();
		boolean bool = localMailSend.sendSysInternalMail("1", t_ccMeetingNotice, null, mailTitle, mailContent);
	}
