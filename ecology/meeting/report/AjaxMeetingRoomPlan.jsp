...
<%!
public String getTimesBg(int j){
	int totalminutes = (j+1) * 30;
...
public String getTimesEd(int j){
	int totalminutes = (j+1) * 30;
	int hours = (int)totalminutes/60;
	int minute = totalminutes%60;
	if(minute==0){
...
{
	String[] minute = new String[24 * 60];
	String starttime1 = dspUnix==2?getTimesBg(Util.getIntValue(thisHour)-1):thisHour+":00";
	String endtime1 = dspUnix==2?getTimesEd(Util.getIntValue(thisHour)):thisHour+":59"; 
...
//图片数据html获取方法
private String getHtml(String roomId){
	String htmlStr = "";
	...
	if(!roomId.equals("")){		
		//查询会议室信息sql
		String selMeetingRoomInfoSql = "select mr.*,h.lastname from MeetingRoom mr,hrmresource h where h.id = mr.hrmid and mr.id = "+roomId;	
		...
		recordSet.executeSql(selMeetingRoomInfoSql);
		if(recordSet.next()){
							        				
			roomName = Util.null2String(recordSet.getString("name"));
			humNum = Util.null2String(recordSet.getString("humnum"));
			hrmid = Util.null2String(recordSet.getString("lastname"));				        				
			...
			imageFilePath = Util.null2String(recordSet.getString("imagefilepath"));
			
			equipment = Util.null2String(recordSet.getString("equipment"));
		}
		baseBean.writeLog("imageFileId:"+imageFileId);
		baseBean.writeLog("imageFileName:"+imageFileName);
		baseBean.writeLog("imageFilePath:"+imageFilePath);
		
		htmlStr += "  ";
		htmlStr += " <table> ";
		htmlStr += " <tr> ";
		htmlStr += " 	<td>会议室名称："+roomName+"</td> ";
		htmlStr += " </tr> ";
		htmlStr += " <tr> ";
		htmlStr += " 	<td>容纳人数："+humNum+"</td> ";
		htmlStr += " </tr> ";
		htmlStr += " <tr> ";
		htmlStr += " 	<td>负责人员："+hrmid+"</td> ";
		htmlStr += " </tr> ";
		htmlStr += " <tr> ";
		htmlStr += " 	<td>会议室设备："+equipment+"</td> ";
		htmlStr += " </tr> ";
		htmlStr += " <tr>  ";
		htmlStr += " 	<td>平面图：</td> ";
		htmlStr += " </tr> ";
		...
		if(!imageFilePath.equals("")) {
			htmlStr += " <td><img src=\""+imageFilePath+"\" width=\"645px\" height=\"330px\" /></td> ";
		}else{
			htmlStr += " <td>无</td> ";
		}
		htmlStr += " </tr> ";
	...
	baseBean.writeLog("htmlStr:"+htmlStr);
	return htmlStr;
}
%>
...
//dgq 过滤更更多
String ismore =  Util.null2String(request.getParameter("ismore"));
String sqlwhere = "";
...
if(subids > 0){
	...
	//如果没有选择过滤分部判断是否是更多
	if(ismore.equals("0") && userSub > 0){
		sqlwhere = "and a.subCompanyId = "+ userSub ;
	}
}
			...
			<% if(bywhat==2) {%>
				...
				<table class=M1 width="100%" border=0 cellspacing=0 cellpadding=0 Style="width:100%; font-size:8pt;table-layout:fixed">
				   ...
				   %>
				        ...
				        <!-- 上传图片 -->
				        		<%
				        			String msgShowDivId = "msgDiv_"+tmproomid;
				        			//查询会议室信息sql
				        			String html = getHtml(tmproomid);
				        			
				        		 %>
								<td class="roomnames" >
								<div class="tdfcs"></div>
								<div class="tdtxt" onMouseOver="javascript:showMsgDiv(this,'<%=msgShowDivId %>');" onMouseOut="hideMsgDiv(this,'<%=msgShowDivId %>')" onclick="showRoomMeetingList('','',<%=tmproomid %>,event)">&nbsp;<%=Util.forHtml(Util.toScreen(MeetingRoomComInfo.getMeetingRoomInfoname(""+tmproomid),user.getLanguage()))%></div>
								<div id="<%=msgShowDivId %>" style="position:absolute;display:none;border:1px solid silver;">
									<%=html %>											
								</div> 
								</td>
					        <%					        
				        ...
				        <tr class="selectable">
			            	...
			            	if (ids.size()==0) { 
			            		for (int p=meetingSetInfo.getTimeRangeStart()*dspUnit ;p<(meetingSetInfo.getTimeRangeEnd()+1)*dspUnit;p++) {
			            			out.println("<td class='tdcls' roomid='"+tmproomid+"' id='"+tmproomid+"' style=\"color:#fff;"+(dspUnit==2&&p%2!=0?"border-left: 0px;":"")+"\"  onselectstart='return false' target='"+p+"' onmousedown='createMeetingAction.startDrag(event);' >&nbsp;</td>");
			            		}
					        ...
					        for(int j=meetingSetInfo.getTimeRangeStart()*dspUnit; j<(meetingSetInfo.getTimeRangeEnd()+1)*dspUnit; j++){
								...
								String tempTimeBg = datenow+" "+(dspUnit==2?getTimesBg(j-1):Util.add0(j,2)+":00") ;	
								String tempTimeed = datenow+" "+(dspUnit==2?getTimesEd(j):Util.add0(j,2)+":59") ;	
								
								System.out.println(tempTimeBg+"  --  "+tempTimeed);
                            ...
                            %>
						         <td class="tdcls" roomid="<%=tmproomid %>" style="<%=(dspUnit==2&&j%2!=0?"border-left: 0px;":"")%>" align=center bgcolor="<%=bgcolor%>" id="<%=tmproomid %>" target="<%=j %>" onselectstart="return false" onmousedown='createMeetingAction.startDrag(event);'  <%if(!"".equals(tdTitle)) {%> title="<%=tdTitle%>"<%}%>><%if(cnt>1) {%><%=cnt %><%} else {%>&nbsp;<%} %></td>
					        <%}%> 
							</tr>
							<%  }%>
						</table>
