...
int count1 = 0;
RecordSet.executeSql("select count(*) as count1 from MQEvaluate where meetingid = " + meetingid);
if(RecordSet.next()){
	count1 = Util.getIntValue(RecordSet.getString("count1"));
}
...
<table style="margin: 0 auto;" width="98%" class="border-table">
	<tr height="40" style="">
		<td colspan="3" style="text-align: center;vertical-align: middle;border: none;">
			<span style="font-size: 16px;color: black;font-family: 微软雅黑;font-weight: 700;">会议效果评估综合得分情况（共<%=count1 %>人评估）</span>
		</td>
	</tr>
	<tr height="40">
	...
	while(RecordSet.next()){
		...
		sqlStr.append("select avg(mqScore) as mqScore from MQEvaluateDet");
		sqlStr.append(" where meetingid = " + meetingid);
		sqlStr.append(" and mqsid = " + id);
		rs.executeSql(sqlStr.toString());
		if(rs.next()){
			mqScore = Util.getDoubleValue(rs.getString("mqScore"));
		}
		BigDecimal mqScoredb = new BigDecimal(mqScore);
		double _mqScore = Util.getDoubleValue(mqScoredb.setScale(1,BigDecimal.ROUND_HALF_UP).toString());
	%>
	<tr height="40">
		...
		<td style="text-align: center;vertical-align: middle">
		  	<div style="float: left;width:40%;text-align: center;margin-bottom: -10px;margin-left: 5px;margin-top: 8px;">
	       	...
	       	<div style="float: left;width:30%;margin-bottom: -10px;margin-top: 18px;margin-left: -50px;">
		 	    该项平均得分：<%=_mqScore %> 分
		  	</div>
		</td>
	</tr>
	...
	<tr height="40">
		...
		<td style="vertical-align: middle;" width="40%">
	 		...
	 		int t_count = 1;
	 		String remark = "";
	 		RecordSet.executeSql("select remark from MQEvaluate where meetingid = " + meetingid + " order by createDate asc,createTime asc");
	 		while(RecordSet.next()){
	 			String ttremark = Util.null2String(RecordSet.getString("remark"));
	 			if(!"".equals(ttremark)){
	 				remark += t_count + ". <SPAN id='remarkSpan' style='margin-left: 7px;margin-top: 5px;margin-bottom: 5px;'>" + ttremark + "</span><br/>";
	 				t_count ++;
	 			}
	 		}
	 		%>
