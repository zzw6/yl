...
{	
    ...
    String description = "您有会议: "+name+"   会议时间:"+begindate+" "+begintime+" 会议地点:"+MeetingInterval.getMeetingRoomInfoname(""+address)+customizeAddress;
	        ...
	        if(repeatType == 0){
				...
				//系统提醒流程 会议通知抄送人 (ccmeetingnotice) 与 会议纪要抄送人 (ccmeetingminutes) by lq 2015-11-5 start
					...
					if(!"".equals(ccMeetingNotice)){
						...
						String mailTitle = description.replaceAll("您有会议:","会议抄送通知:");
    ...
    if(!meetingid.equals("")) {
		...
		//系统提醒流程 会议通知抄送人 (ccmeetingnotice) 与 会议纪要抄送人 (ccmeetingminutes) by lq 2015-11-5 start
			...
			if(!"".equals(ccMeetingNotice)){
				...
				//邮件发送  2017-1-17 start
				BaseBean.writeLog("===========================MeetingOperation.jsp(会议通知抄送人提醒邮件发送) start===========================");
				String mailTitle = tzTitle1.replaceAll("您有会议:","会议抄送通知:");
				String mailContent = "以下是提醒内容，请点击查看详情：<br>";
	...
	if(!approver.equals("0") && !approver.equals("") && !approver.equals(CurrentUser)){
		...
		if(repeatType == 0){
			...
			if(!SWFAccepter.equals("")){
				...
				SWFTitle +=" 会议地点:"+MeetingInterval.getMeetingRoomInfoname(""+address)+customizeAddress;
				SWFRemark="";
