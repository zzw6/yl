...
if("chkRoom".equals(method)){
	...
	String tmid = Util.null2String(fu.getParameter("mid"));
	...
	if(meetingSetInfo.getRoomConflictChk() == 1 ){
		...
		String chkRoomSql = "select address,begindate,enddate,begintime,endtime,id,meetingtype from meeting where meetingstatus in (1,2) " +
				"and repeatType = 0 and isdecision<2 and (cancel is null or cancel<>'1') and (begindate <= '"+enddate+"' and enddate >='"+begindate+"')";
		...
		while(RecordSet.next()) {
			...
			if(!"".equals(meetingaddress)  && !mid.equals(meetingids)) {
				...
				if((str1.compareTo(str2) < 0 && str3.compareTo(str4) > 0)) {
					String[] split = addresstmp.split(",");
					String[] split2=meetingaddress.split(",");
