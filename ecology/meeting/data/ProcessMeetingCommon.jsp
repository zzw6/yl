...
if(!"1".equals(tab)){
	//response.sendRedirect("/meeting/data/ViewMeetingCommonTab.jsp?meetingid="+meetingid) ;
	out.println("<script>wfforward(\"/meeting/data/ViewMeetingCommonTab.jsp?meetingid="+meetingid+"\");</script>");
	return;
}
...
if(!meetingstatus.equals("2")){
    ...
    out.println("<script>wfforward(\"/meeting/data/ViewMeetingCommonTab.jsp?needRefresh="+needRefresh+"&meetingid="+meetingid+"&showdiv="+showDiv+"\");</script>");
	return;
}
...
if(meetingstatus.equals("2") || meetingstatus.equals("5")){ //0：草稿,1：待审批,2:正常,3:退回,4:取消,5:结束
	...
	//System.out.println(MeetingShareUtil.containUser(allUser,RecordSet.getString("membermanager")));
    long nsn = com.weaver.formmodel.util.DateHelper.getMinutesBetween(begindate+"/"+begintime+":00",CurrentDate+"/"+CurrentTime+":00");
	if(nsn>0){
...
<!-- begin 会议议程 -->
<wea:group context="<%=SystemEnv.getHtmlLabelName(31327, user.getLanguage()) %>" attributes="{'groupDisplay':''}">
	<wea:item>
		<div id="" style="display:block;border-left: solid 1px #E6E6E6;">
